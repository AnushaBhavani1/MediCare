...
// Converts input into a clean array; returns [] if invalid
...
// Converts slot strings into a map of YYYY-MM-DD → [times]
      ...
      try {
        ...
        if (up?.secure_url) {
          ...
          if (existing.imagePublicId) {
            try { await deleteFromCloudinary(existing.imagePublicId); } 
            catch (err) { console.warn("Cloudinary delete failed:", err?.message || err); }
          }
        }
      } catch (err) {
        console.error("Cloudinary upload error:", err);
...
export async function deleteService(req, res) {
  try {
    ...
    if (existing.imagePublicId) {
      try { await deleteFromCloudinary(existing.imagePublicId); } 
      catch (err) { console.warn("Failed to delete image from cloudinary:", err?.message || err); }
    }
